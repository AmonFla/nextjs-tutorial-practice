This is a starter template for [Learn Next.js](https://nextjs.org/learn).

## Useful links
### Link
- [File System Routing](https://nextjs.org/docs/api-reference/next/image)
- [Link Api](https://nextjs.org/docs/api-reference/next/link)
### Images
- [Optimization](https://nextjs.org/docs/basic-features/image-optimization)
- [Image Api](https://nextjs.org/docs/api-reference/next/image)
### Head
- [Head Api](https://nextjs.org/docs/api-reference/next/head)
### Script
- [Document](https://nextjs.org/docs/basic-features/script)
### Css
- [Utility First Fundamentals](https://tailwindcss.com/docs/utility-first)
- [clsx Library to toggle classes](https://github.com/lukeed/clsx)
- [PostCSS](https://postcss.org/)
- [Customizing PostCss Config](https://nextjs.org/docs/advanced-features/customizing-postcss-config)
- [Tailwind CSS](https://tailwindcss.com/)
### Sass
- [Sass Support](https://nextjs.org/docs/basic-features/built-in-css-support#sass-support)
### Pre rendering and Data Fetching
- [Pre rendering](https://nextjs.org/docs/basic-features/pages#pre-rendering)
- [getStaticProps](https://nextjs.org/docs/basic-features/data-fetching#getstaticprops-static-generation)
- [getServerSideProps](https://nextjs.org/docs/basic-features/data-fetching#getserversideprops-server-side-rendering)
### Client Side Rendering
- [More info](https://nextjs.org/docs/basic-features/data-fetching#fetching-data-on-the-client-side)
- [SWR Hook](https://swr.vercel.app/)